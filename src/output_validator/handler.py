...
def handler(event: dict[str, Any], context: LambdaContext) -> dict[str, Any]:
    ...
    try:
        ...
        with tracer.capture_method("validate_hls"):
        ...
        if settings.enable_dash:
            with tracer.capture_method("validate_dash"):
        ...
        with tracer.capture_method("validate_duration"):
...
def _validate_dash_outputs(
    ...
    # Check 3: Verify segment files exist
    m4s_files = [f for f in files if f.endswith(".m4s")]
    result["checks"].append({
        "check": "segment_files",
        "passed": len(m4s_files) > 0,
        "message": f"Found {len(m4s_files)} segment file(s)",
    })
